...
function handleTick(evt) {
	...
	{	
		...
		//controller bar x and y position changed according to stage size
		if(window.parent.pageNewAnim == "false") {
			
		mainContainer.y = obj.controlY ? parseFloat(obj.controlY) : 377;
		...
		mainContainer.x = obj.controlX ? parseFloat(obj.controlX) : 20
		mainContainer.scaleX = 1;
		mainContainer.scaleY = 0.85;
		}
	
		else{
			
		mainContainer.y = obj.controlY ? parseFloat(obj.controlY) : 425;
		if(window.parent.pageType=="text-on-left")
		mainContainer.x = obj.controlX ? parseFloat(obj.controlX) : 115
		if(window.parent.pageType=="text-on-right")
		mainContainer.x = obj.controlX ? parseFloat(obj.controlX) : 125
...
(SyntaxComp = function (o) {
	...
	{		
		...
		{	
			...
			{
				$("#"+o.compId).append('<div  style="line-height:20px;">'+d.find("feedback").text()+"</div>");
				t.cl();
				$("#"+o.compId+"_prompt_"+cC).attr("id","");
				t.cd();
				t.pc();	
